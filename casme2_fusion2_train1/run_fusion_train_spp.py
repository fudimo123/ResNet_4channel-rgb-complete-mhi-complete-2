...
from fusion_model_spp import create_fusion_model as create_spp_model
...
def main():
    ...
    result_dir = os.path.join(current_dir, 'fusion_result2')
    ...
    # 覆盖模型工厂为 SPP 版本，并将结果目录改为 fusion_result2
    ft.create_fusion_model = create_spp_model
    ft.RESULT_DIR = 'fusion_result2'

    # 运行训练（LOSO），结果会保存在当前目录 fusion_result2 下
    ft.main()
