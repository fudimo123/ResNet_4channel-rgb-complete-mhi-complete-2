...
from fusion_model_cross_attention import create_fusion_model as create_cross_attn_model
...
def main():
    ...
    result_dir = os.path.join(current_dir, 'fusion_result4')
    ...
    # 覆盖模型工厂为 Cross-Attention 版本，并将结果目录改为 fusion_result4
    ft.create_fusion_model = create_cross_attn_model
    ft.RESULT_DIR = 'fusion_result4'

    # 运行训练（LOSO），结果会保存在当前目录 fusion_result4 下
    ft.main()
