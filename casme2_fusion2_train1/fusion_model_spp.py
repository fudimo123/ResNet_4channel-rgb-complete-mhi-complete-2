...
class SpatialPyramidPooling(nn.Module):
    """
    Simple SPP module using adaptive pooling to aggregate multi-scale features.
...
class LateFusionResNet(nn.Module):
    ...
    at the feature level before the final classification layer, with SPP added to each branch.
    """
    ...
    def _create_resnet_branch(self, in_channels, pretrained=True):
        ...
        if pretrained:
            ...
            if in_channels != 3:
                ...
                with torch.no_grad():
                    if in_channels == 1:
                        # For single channel, use the mean of RGB weights
...
def create_fusion_model(num_classes=3, dropout_p=0.5, pretrained=True):
    """
    Factory function to create a late fusion ResNet model with SPP branches.
    """
