...
MHI_SAVE_DIR = 'data2'  # Directory to save MHI images with facial alignment
CLIP_GRAD_NORM = 1.0
...
def main():
    ...
    train_transform = transforms.Compose([
        transforms.Resize(train_transform_dict["Resize"]["size"]),
        transforms.RandomHorizontalFlip(p=0.5),
        transforms.RandomRotation(degrees=8),  # Increased rotation
        transforms.ColorJitter(brightness=0.3, contrast=0.3),  # Added brightness/contrast
        transforms.RandomAffine(degrees=0, translate=(0.05, 0.05)),  # Added translation
        transforms.ToTensor(),
    ...
    for run_idx, seed in enumerate(REPRODUCIBILITY_SEEDS):
        ...
        with open(run_log_file, 'w') as log_f:
            ...
            for fold_idx, subject_to_leave_out in enumerate(all_subjects):
                ...
                # Enable MHI saving for first run only to avoid duplicates
                save_mhi = (run_idx == 0)
                mhi_save_path = os.path.join(MHI_SAVE_DIR) if save_mhi else None
                
                train_dataset = CASME2Dataset(train_samples, transform=train_transform, 
                                            save_mhi=save_mhi, mhi_save_dir=mhi_save_path)
                ...
                model = resnet18(num_classes=num_classes, pretrained=True, in_channels=1).to(device)
    ...
    # Disable MHI saving for final training to avoid duplicates
    final_train_dataset = CASME2Dataset(all_samples, transform=train_transform, 
                                      save_mhi=False, mhi_save_dir=None)
    g = torch.Generator()
    g.manual_seed(42)
    ...
    final_model = resnet18(num_classes=len(CLASS_NAMES), pretrained=True, in_channels=1).to(device)
