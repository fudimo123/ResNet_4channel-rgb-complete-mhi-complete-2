...
RAW_VIDEO_DIR = os.path.join(SCRIPT_DIR, '../data/SAMM')
ANNOTATION_FILE = os.path.join(SCRIPT_DIR, '../data/SAMM/SAMM_Micro_FACS_Codes_v2.xlsx')
...
def main():
    ...
    config = {
        ...
        'MODEL': 'Late Fusion ResNet-18 (SAMM) with AdamW, Grad Clip, CosineAnnealingLR',
        'rgb_train_transform': rgb_train_transform_dict,
        'dynamic_train_transform': dynamic_train_transform_dict,
        'fusion_architecture': 'Late fusion at feature level'
    }
    ...
    data_parser = SAMMDataParser(ANNOTATION_FILE, RAW_VIDEO_DIR, EMOTION_MAP)
    ...
    for run_idx, seed in enumerate(REPRODUCIBILITY_SEEDS):
        ...
        for fold_idx, test_subject in enumerate(all_subjects):
            ...
            # Load best model
            best_model_path = os.path.join(RESULT_DIR, f'run_{run_idx+1}_fold_{fold_idx+1}_best.pt')
