...
OUTPUT_PATH = Path(r"D:\Users\fuziyan\PycharmProjects\ResNet_4channel-rgb-complete-mhi-complete-2\paper_visualization\average_training_validation_accuracy_combined.png")
...
def main():
    ...
    for name, path, source_ymin in CURVE_FILES:
        ...
        ax.plot(epochs_train, train_acc, color=train_color, linewidth=2.2, label=f"{name} - Train")
        ax.plot(epochs_val, val_acc, color=val_color, linewidth=2.2, linestyle="--", label=f"{name} - Validation")

    ax.set_title("Average Training and Validation Accuracy", fontsize=20, pad=16)
