...
from single_rgb_dataset import SAMMSingleRGBDataset
from single_rgb_models import create_single_rgb_model


RESULT_DIR = "samm_rgb_single_result"
MODEL_NAME = "plain_resnet18"
TRANSFER_WEIGHTS_PATH = None
PROCESSED_RGB_DIR = "./samm_aligned_rgb_offline"
MANIFEST_FILE = "./samm_single_rgb_manifest.csv"
NUM_EPOCHS = 60
BATCH_SIZE = 16
...
def main():
    ...
    print(f"Starting SAMM RGB single-channel training: {MODEL_NAME}")
    ...
    for run_idx, seed in enumerate(REPRODUCIBILITY_SEEDS):
        ...
        for fold_idx, test_subject in enumerate(all_subjects):
            ...
            train_dataset = SAMMSingleRGBDataset(
                train_samples,
                transform=train_transform,
                frame_selection="random",
                processed_rgb_root=PROCESSED_RGB_DIR,
            )
            val_dataset = SAMMSingleRGBDataset(
                val_samples,
