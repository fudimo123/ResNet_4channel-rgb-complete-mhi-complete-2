...
RESULT_DIR = 'fusion_result3'  # Changed to fusion_result2
...
DYNAMIC_IMAGE_DIR = './dynamic_data'  # Dynamic image directory
CLIP_GRAD_NORM = 1.0
...
class OptimizedCASME2FusionDataset(Dataset):
    ...
    def __init__(self, samples, rgb_transform=None, dynamic_transform=None, mhi_transform=None, dynamic_image_dir=None, mhi_save_dir=None):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            return torch.zeros(3, 224, 224), torch.zeros(3, 224, 224), torch.zeros(1, 224, 224), -1
        ...
        # --- Load Dynamic Image ---
        try:
            # Generate dynamic image filename (matching the generated format: s{subject}_{sequence}.jpg)
            subject_dir = os.path.join(self.dynamic_image_dir, f"sub{subject:02d}" if isinstance(subject, int) else str(subject))
            sequence_name = os.path.basename(sequence_path)
            dynamic_filename = f"s{subject}_{sequence_name}.jpg"
            dynamic_path = os.path.join(subject_dir, dynamic_filename)
            
            # Load dynamic image
            if os.path.exists(dynamic_path):
                dynamic_image = Image.open(dynamic_path).convert('RGB')
                dynamic_image = self._extract_face_roi(dynamic_image)
            else:
                print(f"Dynamic image not found: {dynamic_path}")
                # Fallback: create a blank dynamic image
                dynamic_image = Image.new('RGB', (224, 224), 0)
        ...
        return rgb_image, dynamic_image, mhi_image, label

class StackingFusionResNet(nn.Module):
    """
    堆叠集成晚期融合模型：三个分支各自输出 logits，
    将三组 logits 进行拼接后交由元学习器（meta-learner）进行最终决策。
    """
    def __init__(self, num_classes=3, dropout_p=0.5, pretrained=True):
        super(StackingFusionResNet, self).__init__()
        
        # 与现有模型保持一致的三分支特征提取与分类头
        from torchvision.models import resnet18 as torchvision_resnet18
        
        def _create_resnet_branch(in_channels, pretrained=True):
            if pretrained:
        ...
        # RGB/Dynamic/MHI 分支
        self.rgb_branch = _create_resnet_branch(in_channels=3, pretrained=pretrained)
        self.dynamic_branch = _create_resnet_branch(in_channels=3, pretrained=pretrained)
        self.mhi_branch = _create_resnet_branch(in_channels=1, pretrained=pretrained)
    ...
    def forward(self, rgb_input, dynamic_input, mhi_input):
        ...
        # 堆叠：拼接三路 logits 后交由元学习器输出最终分类logits
        stacked_logits = torch.cat([rgb_logits, dynamic_logits, mhi_logits], dim=1)
        fused_logits = self.meta_learner(stacked_logits)
        return fused_logits


def create_stacking_fusion_model(num_classes=3, dropout_p=0.5, pretrained=True):
    return StackingFusionResNet(num_classes=num_classes, dropout_p=dropout_p, pretrained=pretrained)
    ...
    config = {
        ...
        'MODEL': 'Stacking Ensemble Fusion ResNet-18 (RGB, Dynamic, MHI)',
        ...
        'fusion_architecture': 'Stacking ensemble at logits level (concat [logits_rgb, logits_dyn, logits_mhi] -> meta-learner -> 3 classes)',
        'optimizations': {
    ...
    # moved training logic into main()
...
def main():
    ...
    for run_idx, seed in enumerate(REPRODUCIBILITY_SEEDS):
        ...
        for fold_idx, test_subject in enumerate(all_subjects):
            ...
            # Create model
            model = create_stacking_fusion_model(num_classes=len(CLASS_NAMES), dropout_p=0.5, pretrained=True).to(device)
            ...
            for epoch in range(NUM_EPOCHS):
                ...
                for rgb_images, dynamic_images, mhi_images, labels in train_loader:
                    rgb_images, dynamic_images, mhi_images, labels = rgb_images.to(device), dynamic_images.to(device), mhi_images.to(device), labels.to(device)
                    ...
                    optimizer.zero_grad()
                    outputs = model(rgb_images, dynamic_images, mhi_images)
                ...
                with torch.no_grad():
                    for rgb_images, dynamic_images, mhi_images, labels in val_loader:
                        rgb_images, dynamic_images, mhi_images, labels = rgb_images.to(device), dynamic_images.to(device), mhi_images.to(device), labels.to(device)
                        outputs = model(rgb_images, dynamic_images, mhi_images)
                        loss = criterion(outputs, labels)
    ...
    final_train_dataset = OptimizedCASME2FusionDataset(all_samples, 
                                                    rgb_transform=rgb_train_transform, 
                                                    dynamic_transform=rgb_train_transform,
                                                    mhi_transform=mhi_train_transform,
    ...
    final_model = create_stacking_fusion_model(num_classes=len(CLASS_NAMES), dropout_p=0.5, pretrained=True).to(device)
    ...
    for epoch in range(NUM_EPOCHS):
        final_model.train()
        for rgb_images, dynamic_images, mhi_images, labels in final_train_loader:
            rgb_images, dynamic_images, mhi_images, labels = rgb_images.to(device), dynamic_images.to(device), mhi_images.to(device), labels.to(device)
            ...
            outputs = final_model(rgb_images, dynamic_images, mhi_images)
