...
class CASME2FusionDataset(Dataset):
    """
    Dataset class for late fusion training that loads RGB, Dynamic images, and MHI images
    """
    
    def __init__(self, samples, rgb_transform=None, dynamic_transform=None, mhi_transform=None, 
                 dynamic_image_dir=None, mhi_image_dir=None):
        self.samples = samples
        self.rgb_transform = rgb_transform
        self.dynamic_transform = dynamic_transform
        self.mhi_transform = mhi_transform
        self.dynamic_image_dir = dynamic_image_dir or './dynamic_image'
        self.mhi_image_dir = mhi_image_dir or './mhi_image'
    ...
    def __getitem__(self, idx):
        ...
        # --- Load MHI Image ---
        try:
            # Generate MHI image filename based on subject and sequence
            mhi_filename = f"s{subject}_{sequence_name}.jpg"
            subject_dir = f"sub{subject:02d}"
            mhi_image_path = os.path.join(self.mhi_image_dir, subject_dir, mhi_filename)
            
            # Load MHI image
            if os.path.exists(mhi_image_path):
                mhi_image = Image.open(mhi_image_path).convert('L')  # Load as grayscale
                mhi_image = self._extract_face_roi(mhi_image)
            else:
                print(f"MHI image not found: {mhi_image_path}")
                return torch.zeros(3, 224, 224), torch.zeros(3, 224, 224), torch.zeros(1, 224, 224), -1
                
        except (FileNotFoundError, ValueError) as e:
            print(f"Error loading MHI image for {sequence_path}: {e}")
            return torch.zeros(3, 224, 224), torch.zeros(3, 224, 224), torch.zeros(1, 224, 224), -1
