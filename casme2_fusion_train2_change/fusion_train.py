...
TTA_VIEWS = int(os.environ.get('TTA_VIEWS', '1'))
FREEZE_EPOCHS = int(os.environ.get('FREEZE_EPOCHS', '0'))
LR_BACKBONE_MULT = float(os.environ.get('LR_BACKBONE_MULT', '1.0'))
NUM_EPOCHS = 60
BATCH_SIZE = 16
...
def main():
    ...
    for run_idx, seed in enumerate(REPRODUCIBILITY_SEEDS):
        ...
        for fold_idx, test_subject in enumerate(all_subjects):
            ...
            for p in model.fusion_fc.parameters():
                head_ids.add(id(p))
            for p in model.rgb_branch.project.parameters():
                head_ids.add(id(p))
            for p in model.dynamic_branch.project.parameters():
                head_ids.add(id(p))
            backbone_params = [p for p in model.parameters() if id(p) not in head_ids]
            head_params = [p for p in model.parameters() if id(p) in head_ids]
            optimizer = optim.AdamW([
                {'params': backbone_params, 'lr': LEARNING_RATE * LR_BACKBONE_MULT},
            ...
            for epoch in range(NUM_EPOCHS):
                ...
                if epoch < FREEZE_EPOCHS:
                    for p in model.rgb_branch.stem.parameters():
                        p.requires_grad = False
                    for p in model.dynamic_branch.stem.parameters():
                        p.requires_grad = False
                    for p in model.rgb_branch.layer1.parameters():
                        p.requires_grad = False
                    for p in model.dynamic_branch.layer1.parameters():
                        p.requires_grad = False
                else:
                    for p in model.rgb_branch.stem.parameters():
                    ...
                    if USE_EMA:
                        with torch.no_grad():
                            for ema_p, p in zip(ema_model.parameters(), model.parameters()):
                                ema_p.data.mul_(0.999).add_(p.data, alpha=1 - 0.999)
                ...
                with torch.no_grad():
                    for rgb_images, dynamic_images, labels in val_loader:
                        ...
                        if USE_EMA:
                            ...
                            eval_model = model
                        if TTA_VIEWS <= 1:
                            outputs = eval_model(rgb_images, dynamic_images)
                        else:
                            logits_list = []
                            logits_list.append(eval_model(rgb_images, dynamic_images))
                            rgb_flip = torch.flip(rgb_images, dims=[-1])
                            dyn_flip = torch.flip(dynamic_images, dims=[-1])
                            logits_list.append(eval_model(rgb_flip, dyn_flip))
                            outputs = torch.mean(torch.stack(logits_list, dim=0), dim=0)
                        loss = criterion(outputs, labels)
                ...
                if USE_EMA:
                    early_stopping(val_acc, ema_model)
                else:
            ...
            if USE_EMA:
                ema_model.load_state_dict(torch.load(os.path.join(RESULT_DIR, f'run_{run_idx+1}_fold_{fold_idx+1}_best.pt')))
                eval_model = ema_model
            else:
    ...
    final_metrics_file = os.path.join(RESULT_DIR, 'final_metrics.txt')
    with open(final_metrics_file, 'a') as f:
        f.write(f"\n==== Stage: {STAGE_NAME} ====\n")
