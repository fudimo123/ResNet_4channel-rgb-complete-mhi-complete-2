...
from samm_processed_parser import SAMMProcessedDataParser
from early_stopping import EarlyStopping
from focal_loss_ls import FocalLossLabelSmoothing
from metrics import calculate_metrics, plot_confusion_matrix
from single_dynamic_dataset import SAMMSingleDynamicDataset
from single_dynamic_models import create_single_dynamic_model


RESULT_DIR = "samm_dynamic_single_result"
MODEL_NAME = "plain_resnet18"
TRANSFER_WEIGHTS_PATH = None
PROCESSED_DYNAMIC_DIR = "./samm_dynamic_images_offline"
MANIFEST_FILE = "./samm_single_dynamic_manifest.csv"
NUM_EPOCHS = 60
BATCH_SIZE = 16
...
def create_model(num_classes, dropout_p, pretrained):
    return create_single_dynamic_model(
...
def main():
    ...
    data_parser = SAMMProcessedDataParser(MANIFEST_FILE)
    all_samples = data_parser.get_samples()
    all_subjects = data_parser.get_all_subjects()

    print(f"Starting SAMM dynamic single-channel training: {MODEL_NAME}")
    print(
        "Strategy: Offline Dynamic Image + Original Rank Pooling Generation "
        f"+ Label Smoothing ({LABEL_SMOOTHING})"
    )
    print(f"Results will be saved to: {RESULT_DIR}")
    print(f"Using offline dynamic root: {PROCESSED_DYNAMIC_DIR}")
    ...
    for run_idx, seed in enumerate(REPRODUCIBILITY_SEEDS):
        ...
        for fold_idx, test_subject in enumerate(all_subjects):
            ...
            train_dataset = SAMMSingleDynamicDataset(
                train_samples,
                transform=train_transform,
                processed_dynamic_root=PROCESSED_DYNAMIC_DIR,
            )
            val_dataset = SAMMSingleDynamicDataset(
                val_samples,
