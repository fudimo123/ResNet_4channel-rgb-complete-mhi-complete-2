...
# --- Configuration ---
RESULT_DIR = 'fusion_result_smic_se_nomixup'
...
SMIC_RGB_DIR = 'smic_aligned_rgb'
DYNAMIC_IMAGE_DIR = 'smic_dynamic_data_clean'
...
def _freeze_bn_module(module):
    if isinstance(module, (nn.BatchNorm1d, nn.BatchNorm2d)):
        module.eval()

def freeze_batch_norm(model):
    model.apply(_freeze_bn_module)
    for module in model.modules():
        if isinstance(module, (nn.BatchNorm1d, nn.BatchNorm2d)):
            for param in module.parameters():
                param.requires_grad = False

def configure_partial_backbone_freeze(model):
    frozen_prefixes = (
        'rgb_branch.stem',
        'rgb_branch.layer1',
        'rgb_branch.layer2',
        'rgb_branch.layer3',
        'dynamic_branch.stem',
        'dynamic_branch.layer1',
        'dynamic_branch.layer2',
        'dynamic_branch.layer3',
    )
...
def mixup_data(rgb, dyn, labels, alpha, device):
    if alpha <= 0 or rgb.size(0) < 2:
        return rgb, dyn, labels, labels, 1.0
    lam = np.random.beta(alpha, alpha)
    index = torch.randperm(rgb.size(0), device=device)
    mixed_rgb = lam * rgb + (1.0 - lam) * rgb[index]
    mixed_dyn = lam * dyn + (1.0 - lam) * dyn[index]
    labels_a = labels
    labels_b = labels[index]
    return mixed_rgb, mixed_dyn, labels_a, labels_b, float(lam)
...
def train_one_epoch(model, dataloader, criterion, optimizer, device):
    ...
    for rgb, dyn, labels in dataloader:
        ...
        # Dynamic alpha for Focal Loss
        if hasattr(criterion, 'alpha'):
            class_counts = torch.bincount(labels, minlength=3).float()
            weights = 1.0 / (class_counts + 1e-6)
            weights = weights / weights.sum()
            criterion.alpha = weights.to(device)

        optimizer.zero_grad()
        use_mixup = MIXUP_ALPHA > 0 and np.random.rand() < MIXUP_PROB and rgb.size(0) > 1
        if use_mixup:
            rgb, dyn, labels_a, labels_b, lam = mixup_data(rgb, dyn, labels, MIXUP_ALPHA, device)
...
def validate(model, dataloader, criterion, device):
    ...
    epoch_acc = running_corrects.double() / total_samples if total_samples > 0 else 0
    if all_labels:
        epoch_metrics = calculate_metrics(all_labels, all_preds, labels=[0, 1, 2])
    else:
        epoch_metrics = {
            'accuracy': 0.0,
            'uar': 0.0,
    ...
    return epoch_loss, epoch_acc.item(), epoch_metrics

def main():
    print(f"Starting SMIC Fusion Training with SE-Fusion (Adaptive Weighting)...")
        ...
        for i, test_sub in enumerate(subjects):
            ...
            for epoch in range(NUM_EPOCHS):
                ...
                print(
                    ...
                    f"Val UAR: {val_uar:.4f}, Val UF1: {val_uf1:.4f}, "
                    f"Val {MONITOR_METRIC_NAME}: {val_monitor:.4f}"
                )
                ...
                fold_history['val_uar'].append(val_uar)
                fold_history['val_uf1'].append(val_uf1)
                fold_history['val_monitor'].append(val_monitor)

                early_stopping(val_monitor, model)
                if early_stopping.early_stop:
                    print(f"Early stopping triggered on validation {MONITOR_METRIC_NAME}.")
                    break
