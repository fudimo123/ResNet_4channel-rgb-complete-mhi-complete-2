...
from fusion_model_vggface import create_hetero_fusion_model_vggface
...
# --- Configuration ---
RESULT_DIR = 'fusion_result_smic_vggface'
...
def main():
    print("Starting SMIC Heterogeneous Fusion Training (VGGFace2 + ResNet)...")
    print("Architecture: RGB(InceptionResnetV1+CBAM+Grid) + Dyn(ResNet18+SPP) -> Concat -> SE Block")
    print("Strategy: Random Frame RGB + Label Smoothing (0.1)")
    ...
    for run in range(1, NUM_RUNS + 1):
        ...
        for i, test_sub in enumerate(subjects):
            ...
            # Create Model
            model = create_hetero_fusion_model_vggface(
                num_classes=3, 
                dropout_p=0.6, # Keep 0.6 as in your original working config
                pretrained=True, 
