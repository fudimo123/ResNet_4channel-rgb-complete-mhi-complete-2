...
def calculate_metrics(y_true, y_pred, labels=None):
    ...
    """
    # Overall single-label multi-class metrics.
    # Use weighted averages for the generic overall Precision / Recall / F1 block,
    # while keeping UAR / UF1 as macro averages for MEGC-style reporting.
    ...
    # Calculate UAR (Unweighted Average Recall / Macro Recall)
    uar = recall_score(y_true, y_pred, average='macro', zero_division=0)
    
    # Calculate UF1 (Unweighted F1-score / Macro F1)
    # This is the standard metric used in MEGC challenges (Mean of per-class F1)
    uf1 = f1_score(y_true, y_pred, average='macro', zero_division=0)
    
    # Create per-class metrics dictionary for compatibility with save_metrics_with_std
