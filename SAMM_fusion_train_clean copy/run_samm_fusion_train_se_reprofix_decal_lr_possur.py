import os
import samm_fusion_train_decal as ft
from fusion_model_se import create_fusion_model_se
from focal_loss_ls import FocalLossLabelSmoothing


SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
ft.RESULT_DIR = os.path.join(SCRIPT_DIR, 'fusion_result_samm_se_reprofix_decal_lr_possur')
...
def main():
    ...
    print("Starting SAMM Fusion Training with SE-Fusion (Adaptive Weighting, reprofix + static decalcomanie)...")
    print("Architecture: RGB(Grid+CBAM) + Dyn(SPP) -> Concat -> SE Block")
    print("Strategy: Train Random Frame RGB + selective static L/R for positive/surprise + Val/Test Middle Frame RGB")
