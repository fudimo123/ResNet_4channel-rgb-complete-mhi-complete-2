...
ft.RESULT_DIR = os.path.join(SCRIPT_DIR, 'fusion_result_samm_se_reprofix_evm_alpha10_decal_ratio_ps05_sr1')
ft.ALIGNED_RGB_DIR = os.path.join(SCRIPT_DIR, 'samm_aligned_rgb_evm_alpha10')
ft.DYNAMIC_IMAGE_DIR = os.path.join(SCRIPT_DIR, 'samm_dynamic_data_evm_alpha10')

TRANSFER_WEIGHTS_PATH = None

ft.USE_STATIC_AUG = True
ft.STATIC_AUG_DIR = os.path.join(SCRIPT_DIR, 'samm_static_decalcomanie_evm_alpha10')
...
def main():
    ...
    print("Starting SAMM Fusion Training with SE-Fusion (Adaptive Weighting, reprofix + EVM alpha10 + static ratio decalcomanie)...")
    print("Architecture: RGB(Grid+CBAM) + Dyn(SPP) -> Concat -> SE Block")
    print("Strategy: EVM alpha10 RGB/Dynamic + static L/R ratio control (positive=0.5, surprise=1.0) + Val/Test Middle Frame RGB")
