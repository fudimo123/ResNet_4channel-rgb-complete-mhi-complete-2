...
ft.RESULT_DIR = os.path.join(SCRIPT_DIR, 'fusion_result_samm_se_reprofix_evm_classic_butter_o2')
ft.ALIGNED_RGB_DIR = os.path.join(SCRIPT_DIR, 'samm_aligned_rgb_evm_classic_butter_o2')
ft.DYNAMIC_IMAGE_DIR = os.path.join(SCRIPT_DIR, 'samm_dynamic_data_evm_classic_butter_o2')
...
def main():
    ...
    print("Starting SAMM Fusion Training with SE-Fusion (Adaptive Weighting, reprofix + classic_butter_o2)...")
    print("Architecture: RGB(Grid+CBAM) + Dyn(SPP) -> Concat -> SE Block")
    print("Strategy: classic_butter_o2 RGB/Dynamic inputs + Train Random Frame RGB + Val/Test Middle Frame RGB + Label Smoothing (0.1)")
