...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Offline EVM preprocessing for SAMM: align/crop -> EVM -> TIM(32) -> dynamic image."
    )
    parser.add_argument(
        "--raw-dir",
        default=str(project_root / "data" / "SAMM"),
        help="Raw SAMM root directory.",
    )
    parser.add_argument(
        "--excel-path",
        default=str(project_root / "data" / "SAMM" / "SAMM_Micro_FACS_Codes_v2.xlsx"),
        help="SAMM label excel path.",
    )
    parser.add_argument(
        "--output-rgb-dir",
        default=str(script_dir / "samm_aligned_rgb_evm"),
        help="Output directory for EVM-processed aligned RGB sequences.",
    )
    parser.add_argument(
        "--output-dyn-dir",
        default=str(script_dir / "samm_dynamic_data_evm"),
...
def main():
    ...
    for _, row in tqdm(df_clean.iterrows(), total=len(df_clean), desc="Processing SAMM EVM"):
    ...
    summary_path = Path(args.output_rgb_dir).parent / "samm_evm_summary.json"
