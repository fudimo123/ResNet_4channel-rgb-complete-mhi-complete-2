...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Offline EVM preprocessing for SMIC (freq_high=2.5): align/crop -> EVM -> TIM(16) -> dynamic image."
    )
    parser.add_argument(
        "--raw-dir",
    ...
    parser.add_argument(
        "--output-rgb-dir",
        default=str(script_dir / "smic_aligned_rgb_evm_fh25"),
        help="Output directory for EVM-processed aligned RGB sequences.",
    )
    parser.add_argument(
        "--output-dyn-dir",
        default=str(script_dir / "smic_dynamic_data_evm_fh25"),
...
def main():
    ...
    summary_path = Path(args.output_rgb_dir).parent / "preprocess_smic_evm_fh25_summary.json"
