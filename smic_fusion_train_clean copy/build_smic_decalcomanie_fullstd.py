...
def parse_args():
    ...
    parser.add_argument(
        "--output-dir",
        default=str(script_dir / "smic_static_decalcomanie_fullstd"),
        help="Output directory for O/L/R samples built from full standardized sequences.",
    )
    parser.add_argument(
        "--device",
        default="cpu",
...
def process_one_sequence(seq_info, mtcnn, output_root, overwrite=False):
    ...
    standardized_frames = preprocess_sequence(frame_pils, mtcnn)
    left_frames = [make_left_variant(frame) for frame in standardized_frames]
    right_frames = [make_right_variant(frame) for frame in standardized_frames]

    save_variant_frames(standardized_frames, Path(output_root) / "O" / subject / sequence)
    ...
    return {
        ...
        "subject": subject,
        "sequence": sequence,
        "num_frames": len(standardized_frames),
    }
