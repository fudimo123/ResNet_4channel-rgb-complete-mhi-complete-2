...
# --- Configuration ---
RESULT_DIR = 'fusion_result_smic_progressive'
...
SMIC_RGB_DIR = os.path.join(SCRIPT_DIR, 'smic_aligned_rgb')
SMIC_DYNAMIC_DIR = os.path.join(SCRIPT_DIR, 'smic_dynamic_data_clean')
...
def set_parameter_requires_grad(model, feature_extracting):
    if feature_extracting:
        for name, param in model.named_parameters():
            # Freeze the backbone (ResNet layers) but keep the fusion/classification heads trainable
            # We assume your model has rgb_branch and dynamic_branch
            if 'rgb_branch' in name or 'dynamic_branch' in name:
    ...
    for run in range(1, NUM_RUNS + 1):
        ...
        for i, test_sub in enumerate(subjects):
            ...
            train_dataset = SMICFusionDataset(
                ...
                dynamic_image_dir=SMIC_DYNAMIC_DIR
            )
            ...
            # --- Linear Probing: Train ONLY the Fusion Head & Classifier ---
            print("--- Linear Probing: Freezing Backbones permanently ---")
            ...
            optimizer = optim.AdamW(params_to_update, lr=0.0005, weight_decay=1e-4)
            ...
            # Train for up to 60 epochs (only the head)
            for epoch in range(60):
                train_loss, train_acc = train_one_epoch(model, train_loader, criterion, optimizer, device)
                val_loss, val_acc = validate(model, val_loader, criterion, device)

                print(f"[Linear Probing] Epoch {epoch+1}/60: Train Loss: {train_loss:.4f}, Train Acc: {train_acc:.4f}, Val Loss: {val_loss:.4f}, Val Acc: {val_acc:.4f}")
