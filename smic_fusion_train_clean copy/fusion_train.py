...
# --- Configuration ---
RAW_VIDEO_DIR = './smic_aligned_rgb'
DYNAMIC_IMAGE_DIR = './smic_dynamic_data_clean'
RESULT_DIR = 'fusion_result_smic_se_reprofix'
NUM_EPOCHS = 60
BATCH_SIZE = 16
...
def main():
    ...
    for run_idx, seed in enumerate(REPRODUCIBILITY_SEEDS):
        ...
        for fold_idx, test_subject in enumerate(all_subjects):
            test_subject_key = f's{test_subject}'
            train_subjects = [s for s in all_subjects if s != test_subject]
            train_subject_keys = [f's{s}' for s in train_subjects]
            print(f"\n--- Fold {fold_idx+1}/{len(all_subjects)}: Test Subject {test_subject} ---")
            
            # Split data
            train_samples = [s for s in all_samples if s['subject'] in train_subject_keys]
            val_samples = [s for s in all_samples if s['subject'] == test_subject_key]
            ...
            # Create datasets
            train_dataset = SMICFusionDataset(train_samples, 
            ...
            val_dataset = SMICFusionDataset(val_samples, 
            ...
            # Load best model for final evaluation from in-memory early stopping snapshot
            if early_stopping.best_model_state is not None:
                model.load_state_dict(early_stopping.best_model_state)
            else:
                print("Warning: No best model snapshot found; using last epoch weights.")
            model.eval()
            with torch.no_grad():
                for rgb_images, dynamic_images, labels in val_loader:
    ...
    final_train_dataset = SMICFusionDataset(all_samples, 
                                            rgb_transform=rgb_train_transform, 
                                            dynamic_transform=dynamic_train_transform,
                                            dynamic_image_dir=DYNAMIC_IMAGE_DIR,
                                            frame_selection='random')
    ...
    final_model_path = os.path.join(RESULT_DIR, 'fusion_resnet18_smic_final_model_for_gui.pth')
