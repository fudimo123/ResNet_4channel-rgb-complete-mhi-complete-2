...
# --- Configuration ---
RESULT_DIR = 'fusion_result_smic_sam'
...
def train_one_epoch(model, dataloader, criterion, optimizer, device):
    ...
    for rgb, dyn, labels in dataloader:
        ...
        criterion(model(rgb, dyn), labels).backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
...
def main():
    print(f"Starting SMIC Fusion Training with SAM (Sharpness-Aware Minimization)...")
    print(f"Architecture: RGB(Grid+CBAM) + Dyn(SPP) -> Concat -> SE Block")
    print(f"Strategy: Random Frame RGB + Label Smoothing (0.1) + SAM Optimizer (rho=0.05)")
    ...
    for run in range(1, NUM_RUNS + 1):
        ...
        for i, test_sub in enumerate(subjects):
            ...
            optimizer = SAM(model.parameters(), base_optimizer, rho=0.05, lr=0.0001, weight_decay=1e-3) # Increased from 1e-4 to 1e-3
