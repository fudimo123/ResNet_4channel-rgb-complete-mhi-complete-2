...
from fusion_dataset_dynsync import SMICFusionDataset
from early_stopping import EarlyStopping
from metrics import calculate_metrics
from smic_static_augmentation_dynsync import append_augmented_static_samples

# --- Configuration ---
USE_STATIC_AUG = True
BASELINE_RESULT_DIR = 'fusion_result_smic_se_reprofix'
AUG_RESULT_DIR = 'fusion_result_smic_se_reprofix_decal_lr_possur_dynsync'
RESULT_DIR = AUG_RESULT_DIR if USE_STATIC_AUG else BASELINE_RESULT_DIR
...
STATIC_AUG_DIR = 'smic_static_decalcomanie'
STATIC_AUG_DYNAMIC_DIR = 'smic_dynamic_data_decalcomanie'
...
def main():
    ...
    print(f"Strategy: Random Frame RGB (train) + Middle Frame RGB (val/test) + Label Smoothing (0.1)")
    print(f"Static augmentation enabled: {USE_STATIC_AUG}")
    print(f"Static augmentation dir: {STATIC_AUG_DIR} (train only, optional)")
    print(f"Augmented dynamic dir: {STATIC_AUG_DYNAMIC_DIR}")
    print(f"Static augmentation emotions: {STATIC_AUG_EMOTIONS}")
        ...
        for i, test_sub in enumerate(subjects):
            ...
            if USE_STATIC_AUG:
                train_samples = append_augmented_static_samples(
                    ...
                    augmented_dynamic_root=STATIC_AUG_DYNAMIC_DIR
                )
            print(f"Train samples after static augmentation: {len(train_samples)} | Val samples: {len(test_samples)}")
