...
from casme2_data_parser import DSMEDataParser
from fusion_dataset import CASME2FusionDataset
import fusion_model_spp as fusion_model_module
...
RAW_VIDEO_DIR = os.path.join(REPO_ROOT, 'data', 'DSME_pic')
DYNAMIC_IMAGE_DIR = os.path.join(SCRIPT_DIR, 'dsme_dynamic_data')
RESULT_DIR = os.path.join(SCRIPT_DIR, 'fusion_result_dsme_spp')
NUM_EPOCHS = 60
BATCH_SIZE = 16
...
# 三类映射（与CASME2一致）：positive/negative/surprise
CLASS_NAMES = ['positive', 'negative', 'surprise']

# 允许运行脚本覆盖该工厂（例如迁移学习权重）
create_fusion_model = fusion_model_module.create_fusion_model
...
def main():
    ...
    # --- Data Loading ---
    data_parser = DSMEDataParser(RAW_VIDEO_DIR)
    ...
    final_model_path = os.path.join(RESULT_DIR, 'fusion_resnet18_dsme_final_model_for_gui.pth')
