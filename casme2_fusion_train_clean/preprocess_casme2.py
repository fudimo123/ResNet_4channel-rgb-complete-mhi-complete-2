...
# Configuration
RAW_DATA_DIR = r'D:\Users\fuziyan\PycharmProjects\ResNet_4channel-rgb-complete-mhi-complete-2\data\CASME2_RAW_selected\CASME2_RAW_selected'
EXCEL_PATH = r'D:\Users\fuziyan\PycharmProjects\ResNet_4channel-rgb-complete-mhi-complete-2\data\CASME2_RAW_selected\CASME2-coding-20140508.xlsx'
OUTPUT_RGB_DIR = 'casme2_aligned_rgb'
OUTPUT_DYN_DIR = 'casme2_dynamic_data_clean'
TARGET_FRAMES = 32
IMG_SIZE = (224, 224)
MARGIN = 20
...
def process_sequence(seq_path, subject, seq_name, mapped_emotion, onset, offset, mtcnn):
    ...
    for f in all_files:
        try:
            # CASME2 format: img{FrameNum}.jpg (e.g. img46.jpg)
            match = re.search(r'img(\d+)\.jpg', f, re.IGNORECASE)
            if match:
    ...
    if dyn_img is not None:
        ...
        # Format: s{subject}_{seq_name}.jpg (e.g. s01_EP02_01f.jpg)
...
def main():
    ...
    success_count = 0
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing CASME2"):
        # Format subject to '01'
        subject_id = f"{int(row['Subject']):02d}"
        seq_name = str(row['Filename'])
        raw_emotion = str(row['Estimated Emotion']).strip()
        
        # skip if onset or offset are NaN
        ...
        seq_path = os.path.join(RAW_DATA_DIR, f"sub{subject_id}", seq_name)
    ...
    mapping_file = os.path.join(OUTPUT_RGB_DIR, 'casme2_clean_labels.csv')        
    try:
