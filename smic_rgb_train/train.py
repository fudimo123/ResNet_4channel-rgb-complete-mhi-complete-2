...
NUM_EPOCHS = 50
BATCH_SIZE = 16
LEARNING_RATE = 0.001  # Slightly reduced initial learning rate
WEIGHT_DECAY = 1e-4  # Increased weight decay for better regularization
WARMUP_EPOCHS = 5  # Extended warmup period
CLIP_GRAD_NORM = 1.0
...
def main():
    ...
    train_transform_dict = {
        ...
        "RandomRotation": {"degrees": 3},  # Reduced back to 3
        "RandomResizedCrop": {"size": (224, 224), "scale": (0.8, 1.0)},  # Less aggressive cropping
        "ColorJitter": {"brightness": 0.1, "contrast": 0.1, "saturation": 0.1, "hue": 0.05, "p": 0.3},
        "RandomAffine": {"degrees": 3, "translate": (0.05, 0.05), "scale": (0.95, 1.05), "p": 0.2},
    ...
    for run_idx, seed in enumerate(REPRODUCIBILITY_SEEDS):
        ...
        with open(run_log_file, 'w') as log_f:
            ...
            for fold_idx, subject_to_leave_out in enumerate(all_subjects):
                ...
                criterion = FocalLoss(gamma=1.5) # Increased gamma for better hard example mining
                optimizer = optim.AdamW(model.parameters(), lr=LEARNING_RATE, weight_decay=WEIGHT_DECAY)
                
                # Enhanced learning rate scheduling with cosine annealing and restarts
                warmup_scheduler = LambdaLR(optimizer, lr_lambda=lambda epoch: (epoch + 1) / WARMUP_EPOCHS if epoch < WARMUP_EPOCHS else 1)
                main_scheduler = CosineAnnealingLR(optimizer, T_max=(NUM_EPOCHS - WARMUP_EPOCHS) // 2, eta_min=LEARNING_RATE * 0.1)
    ...
    final_criterion = FocalLoss(gamma=1.5)  # Consistent with CV training
    ...
    final_model_path = os.path.join(RESULT_DIR, 'resnet18_smic_final_model_for_gui.pth')
