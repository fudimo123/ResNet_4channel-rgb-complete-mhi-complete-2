            ...
            if os.path.exists(micro_dir):
                for emotion in os.listdir(micro_dir):
                    ...
                    for sequence in os.listdir(emotion_dir):
                        ...
                        # 对于SMIC数据集，我们使用所有帧作为onset到offset的范围
                        frame_numbers = []
                        for img_file in image_files:
                            try:
                                # 提取帧号，假设文件名格式为reg_imageXXXXXX.bmp
                                frame_num = int(img_file.split('reg_image')[1].split('.')[0])
                        ...
                        frame_numbers.sort()
                        onset = frame_numbers[0]
                        offset = frame_numbers[-1]
                        apex = frame_numbers[len(frame_numbers)//2]  # 中间帧作为apex
