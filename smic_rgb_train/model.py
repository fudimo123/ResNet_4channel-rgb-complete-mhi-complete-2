...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, in_channels=3, dropout_p=0.3):
        ...
        self.dropout1 = nn.Dropout2d(p=0.05)  # Reduced spatial dropout after layer1
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2)
        self.dropout2 = nn.Dropout2d(p=0.05)  # Reduced spatial dropout after layer2
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2)
        self.cbam3 = CBAM(256)  # Attention only after layer3
        self.dropout3 = nn.Dropout2d(p=0.05)  # Reduced spatial dropout after layer3
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2)
        self.cbam4 = CBAM(512)  # Attention after layer4
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        # Simplified classifier with reduced dropout
        self.dropout_pre = nn.Dropout(p=dropout_p * 0.5)
        self.fc_intermediate = nn.Linear(512 * block.expansion, 256)
        self.bn_fc = nn.BatchNorm1d(256)
        self.dropout_post = nn.Dropout(p=dropout_p * 0.7)
