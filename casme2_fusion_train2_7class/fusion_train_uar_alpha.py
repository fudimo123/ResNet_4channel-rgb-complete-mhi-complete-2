...
def main():
    ...
    config = {
        ...
        'fusion_architecture': 'Late fusion at feature level (RGB: 512 + Dynamic: 512 -> 1024 -> 7 classes)'
    }
    ...
    for run_idx, seed in enumerate(REPRODUCIBILITY_SEEDS):
        ...
        for fold_idx, test_subject in enumerate(all_subjects):
            ...
            inv = 1.0 / np.where(counts > 0, counts, 1.0)
            inv = inv / inv.sum()
    ...
    counts_all = np.bincount([s['label'] for s in all_samples], minlength=len(CLASS_NAMES)).astype(np.float32)
    inv_all = 1.0 / np.where(counts_all > 0, counts_all, 1.0)
    inv_all = inv_all / inv_all.sum()
    final_alpha = torch.tensor(inv_all, dtype=torch.float32, device=device)
    final_criterion.alpha = final_alpha
    for epoch in range(NUM_EPOCHS):
        final_model.train()
