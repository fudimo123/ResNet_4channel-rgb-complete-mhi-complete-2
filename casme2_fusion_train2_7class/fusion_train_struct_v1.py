...
from fusion_model_spp_struct_v1 import create_fusion_model
...
def main():
    ...
    config = {
        ...
        'MODEL': 'Late Fusion ResNet-18 + SPP + Gated MLP Head (StructV1)',
        'rgb_train_transform': rgb_train_transform_dict,
        'dynamic_train_transform': dynamic_train_transform_dict,
        'fusion_architecture': 'Late fusion with gating; 1024→512→7 head; UAR early stop; fixed alpha; logit adjustment'
    }
    ...
    for run_idx, seed in enumerate(REPRODUCIBILITY_SEEDS):
        ...
        for fold_idx, test_subject in enumerate(all_subjects):
            ...
            train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=False, sampler=sampler, num_workers=0, pin_memory=True, worker_init_fn=seed_worker)
            ...
            early_stopping = EarlyStopping(patience=40, verbose=True, path=os.path.join(RESULT_DIR, f'run_{run_idx+1}_fold_{fold_idx+1}_best.pt'))
            # counts computed above for sampler and prior
            ...
            for epoch in range(NUM_EPOCHS):
                ...
                for rgb_images, dynamic_images, labels in train_loader:
                    ...
                    outputs = outputs + LOGIT_TAU * prior_log
                ...
                uar_metric = metrics_epoch['uar']
                early_stopping(uar_metric, model)
