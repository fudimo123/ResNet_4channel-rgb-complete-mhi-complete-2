...
class LateFusionResNetWithSPPStructV1(nn.Module):
    def __init__(self, num_classes=7, dropout_p=0.5, pretrained=True, spp_levels=(1, 2, 4), transfer_weights_path=None):
        super().__init__()
        self.rgb_branch = ResNetBranchWithSPP(
            in_channels=3, pretrained=pretrained, out_dim=512, spp_levels=spp_levels, transfer_weights_path=transfer_weights_path
        )
...
def create_fusion_model(num_classes=7, dropout_p=0.5, pretrained=True):
    return LateFusionResNetWithSPPStructV1(num_classes=num_classes, dropout_p=dropout_p, pretrained=pretrained)

def create_fusion_model_with_transfer(num_classes=7, dropout_p=0.5, pretrained=True, transfer_weights_path=None):
    return LateFusionResNetWithSPPStructV1(num_classes=num_classes, dropout_p=dropout_p, pretrained=pretrained, spp_levels=(1, 2, 4), transfer_weights_path=transfer_weights_path)
