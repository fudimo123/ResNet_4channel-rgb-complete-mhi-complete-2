...
ft.RESULT_DIR = os.path.join(SCRIPT_DIR, 'fusion_result_casme2_se_reprofix_evm_classic_butter_o1')
ft.ALIGNED_RGB_DIR = os.path.join(SCRIPT_DIR, 'casme2_aligned_rgb_evm_classic_butter_o1')
ft.DYNAMIC_IMAGE_DIR = os.path.join(SCRIPT_DIR, 'casme2_dynamic_data_evm_classic_butter_o1')
...
def main():
    ...
    print("Starting CASME2 Fusion Training with SE-Fusion (reprofix + classic_butter_o1)...")
    print("Architecture: RGB(Grid+CBAM) + Dyn(SPP) -> Concat -> SE Block")
    print("Strategy: classic_butter_o1 RGB/Dynamic inputs + Train Random Frame RGB + Val/Test Middle Frame RGB + Label Smoothing (0.1)")
