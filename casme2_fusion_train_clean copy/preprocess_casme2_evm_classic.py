...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Offline classic EVM preprocessing for CASME2: align/crop -> classic EVM -> TIM(32) -> dynamic image."
    )
    parser.add_argument("--raw-dir", default=str(project_root / "data" / "CASME2_RAW_selected" / "CASME2_RAW_selected"))
    parser.add_argument("--excel-path", default=str(project_root / "data" / "CASME2_RAW_selected" / "CASME2-coding-20140508.xlsx"))
    parser.add_argument("--output-rgb-dir", default=str(script_dir / "casme2_aligned_rgb_evm_classic"))
    parser.add_argument("--output-dyn-dir", default=str(script_dir / "casme2_dynamic_data_evm_classic"))
...
def main():
    ...
    for _, row in tqdm(df_clean.iterrows(), total=len(df_clean), desc="Processing CASME2 Classic EVM"):
    ...
    summary_path = Path(args.output_rgb_dir).parent / "casme2_classic_evm_summary.json"
