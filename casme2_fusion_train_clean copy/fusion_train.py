...
MAPPING_FILE = os.path.join(SCRIPT_DIR, 'casme2_clean_labels.csv')
ALIGNED_RGB_DIR = os.path.join(SCRIPT_DIR, 'casme2_aligned_rgb')
DYNAMIC_IMAGE_DIR = os.path.join(SCRIPT_DIR, 'dynamic_data')
...
STATIC_AUG_DIR = os.path.join(SCRIPT_DIR, 'casme2_static_decalcomanie')
STATIC_AUG_VARIANTS = ('L', 'R')
STATIC_AUG_EMOTIONS = ('positive', 'surprise')
STATIC_AUG_KWARGS = {}
APPEND_STATIC_AUG_SAMPLES = None
...
def main():
    ...
    if not os.path.exists(MAPPING_FILE):
        raise FileNotFoundError(
            ...
            "Run preprocess_casme2_aligned_rgb.py first."
        )
    if not os.path.isdir(ALIGNED_RGB_DIR):
        raise FileNotFoundError(
            f"Aligned RGB directory not found: {ALIGNED_RGB_DIR}. "
            "Run preprocess_casme2_aligned_rgb.py first."
        )
    if not os.path.isdir(DYNAMIC_IMAGE_DIR):
        raise FileNotFoundError(
            f"Dynamic image directory not found: {DYNAMIC_IMAGE_DIR}. "
            "Build the required dynamic images before training."
        )
    ...
    config = {
        ...
        'fusion_architecture': 'Late fusion at feature level (RGB: 512 + Dynamic: 512 -> 1024 -> 3 classes)'
    }
    ...
    # --- Data Loading ---
    data_parser = CASME2DataParser(MAPPING_FILE, ALIGNED_RGB_DIR, {name: idx for idx, name in enumerate(CLASS_NAMES)})
    all_samples = data_parser.get_samples()
    all_subjects = data_parser.get_all_subjects()

    if not all_samples or not all_subjects:
        raise RuntimeError(
            "No valid CASME2 samples were loaded from the offline dataset. "
            "Check casme2_clean_labels.csv, aligned RGB folders, and dynamic image files."
        )
    ...
    for run_idx, seed in enumerate(REPRODUCIBILITY_SEEDS):
        ...
        for fold_idx, test_subject in enumerate(all_subjects):
            ...
            if USE_STATIC_AUG and APPEND_STATIC_AUG_SAMPLES is not None:
                train_samples = APPEND_STATIC_AUG_SAMPLES(
                    train_samples,
                    STATIC_AUG_DIR,
            ...
            if len(train_samples) == 0:
                raise RuntimeError(
                    f"No training samples remain for fold {fold_idx + 1} with test subject {test_subject}."
                )
            ...
            # Create datasets
            train_dataset = CASME2FusionDataset(
                train_samples,
            ...
            val_dataset = CASME2FusionDataset(
                val_samples,
    ...
    if not all_runs_metrics or not all_runs_histories:
        raise RuntimeError("Training did not produce any completed runs. Check the offline dataset and training logs.")
