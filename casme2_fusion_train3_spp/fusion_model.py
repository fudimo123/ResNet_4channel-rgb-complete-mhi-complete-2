...
class LateFusionResNet(nn.Module):
    ...
    def __init__(self, num_classes=3, dropout_p=0.5, pretrained=True):
        ...
        # Fusion layer
        # Each branch outputs 512 features, so combined is 1536 (512 * 3)
    ...
    def forward(self, rgb_input, dynamic_input, mhi_input):
        """
        Forward pass through all three branches with dynamic weight adjustment
        ...
        fused_features = torch.cat([rgb_features, dynamic_features, mhi_features], dim=1)  # (batch_size, 1536)
