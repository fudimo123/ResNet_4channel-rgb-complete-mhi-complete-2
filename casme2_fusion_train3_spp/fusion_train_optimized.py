...
RESULT_DIR = 'fusion_result3'  # Save results to fusion_result3
NUM_EPOCHS = 60
...
CLIP_GRAD_NORM = 1.0
REPRODUCIBILITY_SEEDS = [42, 43, 44]  # Multiple seeds for standard multi-run
EMOTION_MAP = {
...
def pregenerate_mhi_data(data_parser, mhi_save_dir):
    ...
    # Limit to the first subject for quick LOSO validation demonstration
...
def main():
    ...
    all_samples = data_parser.get_samples()
    all_subjects = data_parser.get_all_subjects()
    # --- Pre-generate MHI Data (skip in quick run) ---
    if NUM_EPOCHS > 1:
        pregenerate_mhi_data(data_parser, MHI_SAVE_DIR)
    else:
        print("[Quick Run] Skipping MHI pre-generation to speed up the first-epoch demo.")
