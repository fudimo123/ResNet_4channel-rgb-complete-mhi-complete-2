...
# --- Configuration ---
RESULT_DIR = 'fusion_result_smic_se_aug'
...
def get_transforms(is_train=True):
    if is_train:
        return transforms.Compose([
            ...
            transforms.RandomRotation(degrees=15),  # Increased from 5
            transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3, hue=0.1), # Stronger color jitter
            transforms.RandomAffine(degrees=0, translate=(0.1, 0.1), scale=(0.9, 1.1)), # Added translation and scale
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
            transforms.RandomErasing(p=0.5, scale=(0.02, 0.1), ratio=(0.3, 3.3), value=0), # Added RandomErasing (MUST be after ToTensor)
        ])
    else:
        return transforms.Compose([
            transforms.Resize((224, 224)),
...
def mixup_data(x1, x2, y, alpha=0.4): # alpha usually 0.2-0.4 for small datasets
...
def train_one_epoch(model, dataloader, criterion, optimizer, device, use_mixup=True):
    ...
    for rgb, dyn, labels in dataloader:
        ...
        # For accuracy calculation, we compare against the original label (approximate)
        _, preds = torch.max(outputs, 1)
        if use_mixup:
            # In mixup, 'accuracy' is a bit fuzzy, usually we just check if it matches the stronger label
            # or just skip rigorous train acc calculation. Here we compare to targets_a (weighted by lam > 0.5?)
            # Simplified: just use targets_a for rough logging
...
def main():
    print(f"Starting SMIC Fusion Training with Mixup & Enhanced Augmentation...")
    print(f"Architecture: RGB(Grid+CBAM) + Dyn(SPP) -> Concat -> SE Block")
    print(f"Strategy: Mixup (alpha=0.4) + RandomErasing + Stronger Jitter")
