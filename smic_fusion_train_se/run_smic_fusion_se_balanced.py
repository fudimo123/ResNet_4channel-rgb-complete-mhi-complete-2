...
# --- Configuration ---
RESULT_DIR = 'fusion_result_smic_se_balanced'
...
def get_transforms(is_train=True):
    if is_train:
        return transforms.Compose([
            ...
            # Keep the strong augmentation from previous step
...
def train_one_epoch(model, dataloader, criterion, optimizer, device, use_mixup=True):
    ...
    for rgb, dyn, labels in dataloader:
        ...
        # Note: With WeightedRandomSampler, dynamic alpha in Focal Loss might be redundant 
        # or even counter-productive if not careful, but keeping it usually doesn't hurt much.
        # We'll stick to static alpha or let the sampler handle the balancing.
        # Here we disable dynamic alpha adjustment to rely purely on sampling.
...
def main():
    print(f"Starting SMIC Fusion Training with Weighted Sampling, Mixup & Enhanced Augmentation...")
    print(f"Architecture: RGB(Grid+CBAM) + Dyn(SPP) -> Concat -> SE Block")
    print(f"Strategy: WeightedRandomSampler + Mixup + Stronger Jitter")
    ...
    for run in range(1, NUM_RUNS + 1):
        ...
        for i, test_sub in enumerate(subjects):
            ...
            # Explicitly set alpha to None as we are using WeightedRandomSampler for balancing
